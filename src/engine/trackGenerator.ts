...
/**
 * Tạo kết cấu mặt đường Asphalt/Tarmac chân thực chuẩn đồ họa đua xe F1
 * Có vệt cao su bám đường (racing rubbering line), các hạt đá li ti và độ sần chống nhòe
 */
function createAsphaltTexture(textureType: string, baseColorHex: number): THREE.CanvasTexture {
  ...
  // 2. Vi hạt sỏi đá asphalt micro-aggregate (hàng ngàn chấm hạt sần tự nhiên)
  ...
  // 3. Vết cào xước nhẹ theo chiều dọc đường (Tarmac grain lines)
  ctx.fillStyle = 'rgba(0, 0, 0, 0.05)';
  for (let l = 0; l < 80; l++) {
    const lx = Math.random() * 512;
    const lw = 1 + Math.random() * 2;
    ctx.fillRect(lx, 0, lw, 512);
  }

  // 4. Hai vạch biên trắng liền mạch 2 bên mép đường (Solid Continuous Edge Lines) - loại bỏ 100% vạch đứt đoạn
...
/**
 * Bản đồ độ sần BumpMap cho mặt đường để bắt ánh sáng tự nhiên
 */
function createAsphaltBumpTexture(): THREE.CanvasTexture {
  const canvas = document.createElement('canvas');
  canvas.width = 256;
  canvas.height = 256;
  const ctx = canvas.getContext('2d')!;
  ...
  const data = imgData.data;
  for (let i = 0; i < data.length; i += 4) {
    const n = Math.floor(Math.random() * 90);
    data[i] = 110 + n;
    data[i + 1] = 110 + n;
    data[i + 2] = 110 + n;
  }
...
function createGrassTerrainTexture(groundColorHex: number): THREE.CanvasTexture {
  ...
  const baseCol = new THREE.Color(groundColorHex);
  const r = Math.round(baseCol.r * 255);
  const g = Math.round(baseCol.g * 255);
  const b = Math.round(baseCol.b * 255);
...
/**
 * Tạo bầu trời chuyển sắc quang quyển (Atmospheric Rayleigh Gradient Sky)
 * với ánh sáng chân trời tự nhiên, loại bỏ mảng màu xanh lơ phẳng lì đơn điệu
 */
export function createSkyAtmosphereTexture(skyColorHex: number, fogColorHex: number): THREE.CanvasTexture {
  const canvas = document.createElement('canvas');
  canvas.width = 512;
  canvas.height = 1024;
  const ctx = canvas.getContext('2d')!;
  ...
  const skyR = Math.round(skyColor.r * 255);
  const skyG = Math.round(skyColor.g * 255);
  const skyB = Math.round(skyColor.b * 255);

  const fogR = Math.round(fogColor.r * 255);
  const fogG = Math.round(fogColor.g * 255);
  const fogB = Math.round(fogColor.b * 255);

  // Gradient từ đỉnh thiên đỉnh (Zenith) xuống đường chân trời (Horizon)
  const grad = ctx.createLinearGradient(0, 0, 0, 1024);
  grad.addColorStop(0.0, `rgb(${Math.max(0, skyR - 45)}, ${Math.max(0, skyG - 35)}, ${Math.min(255, skyB + 15)})`);
  grad.addColorStop(0.45, `rgb(${skyR}, ${skyG}, ${skyB})`);
  grad.addColorStop(0.78, `rgb(${Math.round(skyR * 0.55 + fogR * 0.45)}, ${Math.round(skyG * 0.55 + fogG * 0.45)}, ${Math.round(skyB * 0.55 + fogB * 0.45)})`);
  grad.addColorStop(0.92, `rgb(${fogR}, ${fogG}, ${fogB})`);
  grad.addColorStop(1.0, `rgb(${fogR}, ${fogG}, ${fogB})`);

  ctx.fillStyle = grad;
  ctx.fillRect(0, 0, 512, 1024);

  // Mây trắng nhẹ nhàng ở chân trời
  ctx.fillStyle = 'rgba(255, 255, 255, 0.22)';
  for (let c = 0; c < 14; c++) {
    const cx = Math.random() * 512;
    const cy = 660 + Math.random() * 200;
    const crx = 70 + Math.random() * 130;
    const cry = 16 + Math.random() * 26;
    ctx.beginPath();
...
export class TrackGenerator {
  ...
  static generateTrack(seed: number, biome: TrackBiome): GeneratedTrack {
    ...
    const trackWidth = 15;
    const segments = 1600; // 1600 segments siêu mịn, khép kín tuyệt đối 100% không đứt đoạn
    ...
    for (let i = 0; i <= segments; i++) {
      const t = i / segments;
      const point = (i === segments) ? safeGetPointAt(curve, 0) : safeGetPointAt(curve, t);
      const tangent = (i === segments) ? safeGetTangentAt(curve, 0) : safeGetTangentAt(curve, t);
      ...
      if (i === segments) {
        // Khép kín mối nối vạch xuất phát / đích tuyệt đối 100% (Watertight zero-gap seam)
    ...
    // =========================================================================
    // 1. VẠCH KẺ ĐƯỜNG BIÊN 2 BÊN (ROAD EDGE LINES) - DẢI RIBBON LIỀN MẠCH 100%
    // Uốn lượn mềm mại theo đường cong, triệt tiêu hoàn toàn góc vuông và đứt đoạn
    // =========================================================================
    const edgeSegments = segments;
    const edgeLineWidth = 0.45;
    const leftEdgePositions: number[] = [];
    const rightEdgePositions: number[] = [];
    const edgeIndices: number[] = [];

    for (let i = 0; i <= edgeSegments; i++) {
      const t = i / edgeSegments;
      const point = (i === edgeSegments) ? safeGetPointAt(curve, 0) : safeGetPointAt(curve, t);
      const tangent = (i === edgeSegments) ? safeGetTangentAt(curve, 0) : safeGetTangentAt(curve, t);
      let normal = new THREE.Vector3().crossVectors(tangent, up);
      if (normal.lengthSq() < 0.0001) normal = new THREE.Vector3(1, 0, 0);
      else normal.normalize();

      // Vạch biên trái
      const pL_inner = point.clone().addScaledVector(normal, trackWidth / 2 - 0.25 - edgeLineWidth);
      const pL_outer = point.clone().addScaledVector(normal, trackWidth / 2 - 0.25);

      // Vạch biên phải
      const pR_inner = point.clone().addScaledVector(normal, -trackWidth / 2 + 0.25 + edgeLineWidth);
      const pR_outer = point.clone().addScaledVector(normal, -trackWidth / 2 + 0.25);

      if (i === edgeSegments) {
        // Khép kín tuyệt đối tại điểm nối loop (watertight seam)
        leftEdgePositions.push(leftEdgePositions[0], leftEdgePositions[1], leftEdgePositions[2]);
        leftEdgePositions.push(leftEdgePositions[3], leftEdgePositions[4], leftEdgePositions[5]);
        rightEdgePositions.push(rightEdgePositions[0], rightEdgePositions[1], rightEdgePositions[2]);
        rightEdgePositions.push(rightEdgePositions[3], rightEdgePositions[4], rightEdgePositions[5]);
      } else {
        leftEdgePositions.push(pL_inner.x, pL_inner.y + 0.12, pL_inner.z);
        leftEdgePositions.push(pL_outer.x, pL_outer.y + 0.12, pL_outer.z);
        rightEdgePositions.push(pR_inner.x, pR_inner.y + 0.12, pR_inner.z);
        rightEdgePositions.push(pR_outer.x, pR_outer.y + 0.12, pR_outer.z);
      }

      if (i < edgeSegments) {
        const base = i * 2;
        edgeIndices.push(base, base + 1, base + 2);
        edgeIndices.push(base + 1, base + 3, base + 2);
      }
    }

    const leftEdgeGeo = new THREE.BufferGeometry();
    leftEdgeGeo.setAttribute('position', new THREE.Float32BufferAttribute(leftEdgePositions, 3));
    leftEdgeGeo.setIndex(edgeIndices);
    leftEdgeGeo.computeVertexNormals();

    const rightEdgeGeo = new THREE.BufferGeometry();
    rightEdgeGeo.setAttribute('position', new THREE.Float32BufferAttribute(rightEdgePositions, 3));
    rightEdgeGeo.setIndex(edgeIndices);
    rightEdgeGeo.computeVertexNormals();
    ...
    const leftEdgeMesh = new THREE.Mesh(leftEdgeGeo, edgeLineMat);
    const rightEdgeMesh = new THREE.Mesh(rightEdgeGeo, edgeLineMat);
    leftEdgeMesh.frustumCulled = false;
    rightEdgeMesh.frustumCulled = false;
    ...
    const grassTex = createGrassTerrainTexture(biome.groundColor || 0x22c55e);
