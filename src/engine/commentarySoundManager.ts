...
class CommentarySoundManager {
  ...
  public setSharedAudioContext(ctx: AudioContext) {
  ...
  /**
   * Tải và giải mã ngay lập tức 1 clip nếu chưa có trong bộ nhớ cache
   */
  async getOrLoadClipAsync(clipId: string): Promise<DecodedCommentaryClip | null> {
    const existing = this.decodedClips.get(clipId);
    if (existing && existing.audioBuffer) return existing;

    const info = COMMENTARY_CLIPS.find(c => c.id === clipId);
    if (!info) return null;
    ...
    try {
      ...
      const decoded: DecodedCommentaryClip = {
        info,
        audioBuffer,
      ...
      this.decodedClips.set(info.id, decoded);
      return decoded;
    } catch (err) {
      console.warn(`Lỗi khi load on-demand clip ${clipId}:`, err);
      return null;
    }
  }
  ...
  getTimelineForInstance(instanceId: number, seed: number = 632585, durationSeconds: number = 120, lang: 'vi' | 'en' = 'vi'): ScheduledCommentaryEvent[] {
    ...
    if (cached) {
      // Đảm bảo dữ liệu âm thanh PCM luôn được nạp đầy đủ vào các sự kiện đã cache
      for (const item of cached) {
        if (!item.pcmLeft && item.clipId) {
          const dec = this.decodedClips.get(item.clipId);
          if (dec) {
            item.audioBuffer = dec.audioBuffer;
            item.pcmLeft = dec.pcmLeft;
            item.pcmRight = dec.pcmRight;
            item.sampleRate = dec.sampleRate;
          }
        }
      }
      return cached;
