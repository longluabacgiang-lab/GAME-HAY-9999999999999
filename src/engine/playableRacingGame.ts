...
export class PlayableRacingGame {
  ...
  private buildTrack() {
    ...
    for (let i = 0; i <= segments; i++) {
      ...
      if (i === segments) {
        // Watertight seam closure at lap junction
    ...
    // Dải viền biên đường 2 bên liền mạch 100% (Watertight continuous ribbons)
    const edgeLineWidth = 0.45;
    const leftEdgePos: number[] = [];
    const rightEdgePos: number[] = [];
    const edgeIndices: number[] = [];

    for (let i = 0; i <= segments; i++) {
      const u = i / segments;
      const pt = (i === segments) ? safeGetPointAt(this.trackCurve, 0) : safeGetPointAt(this.trackCurve, u);
      const tg = (i === segments) ? safeGetTangentAt(this.trackCurve, 0) : safeGetTangentAt(this.trackCurve, u);
      let normal = new THREE.Vector3().crossVectors(tg, new THREE.Vector3(0, 1, 0));
      if (normal.lengthSq() < 0.0001) normal = new THREE.Vector3(1, 0, 0);
      else normal.normalize();

      const pL_inner = pt.clone().addScaledVector(normal, -roadHalfWidth + 0.25);
      const pL_outer = pt.clone().addScaledVector(normal, -roadHalfWidth + 0.25 + edgeLineWidth);
      const pR_inner = pt.clone().addScaledVector(normal, roadHalfWidth - 0.25 - edgeLineWidth);
      const pR_outer = pt.clone().addScaledVector(normal, roadHalfWidth - 0.25);

      if (i === segments) {
        leftEdgePos.push(leftEdgePos[0], leftEdgePos[1], leftEdgePos[2]);
        leftEdgePos.push(leftEdgePos[3], leftEdgePos[4], leftEdgePos[5]);
        rightEdgePos.push(rightEdgePos[0], rightEdgePos[1], rightEdgePos[2]);
        rightEdgePos.push(rightEdgePos[3], rightEdgePos[4], rightEdgePos[5]);
      } else {
        leftEdgePos.push(pL_inner.x, pL_inner.y + 0.14, pL_inner.z);
        leftEdgePos.push(pL_outer.x, pL_outer.y + 0.14, pL_outer.z);
        rightEdgePos.push(pR_inner.x, pR_inner.y + 0.14, pR_inner.z);
        rightEdgePos.push(pR_outer.x, pR_outer.y + 0.14, pR_outer.z);
      }
    ...
    leftEdgeGeo.setAttribute('position', new THREE.Float32BufferAttribute(leftEdgePos, 3));
    leftEdgeGeo.setIndex(edgeIndices);
    leftEdgeGeo.computeVertexNormals();

    const rightEdgeGeo = new THREE.BufferGeometry();
    rightEdgeGeo.setAttribute('position', new THREE.Float32BufferAttribute(rightEdgePos, 3));
    rightEdgeGeo.setIndex(edgeIndices);
    rightEdgeGeo.computeVertexNormals();

    const edgeLineMat = new THREE.MeshBasicMaterial({ color: 0xffffff, side: THREE.DoubleSide });
    ...
    this.trackMeshGroup.add(leftEdgeMesh, rightEdgeMesh);
