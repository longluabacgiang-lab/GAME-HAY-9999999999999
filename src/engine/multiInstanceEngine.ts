...
export class MultiInstanceEngine {
  ...
  public activeAudioInstanceId: number = 1;
  private resizeObserver: ResizeObserver | null = null;

  setActiveAudioInstance(instanceId: number) {
    if (this.instances.has(instanceId)) {
      this.activeAudioInstanceId = instanceId;
    }
  }

  getActiveAudioInstanceId(): number {
  ...
  start(config: SystemConfig) {
    ...
    const loop = (currentTime: number) => {
      ...
      if (!this.isPaused) {
        ...
        // Đạo diễn âm thanh không gian 3D theo đúng Luồng Game đang kích hoạt/xem
        const activeInstance = this.instances.get(this.activeAudioInstanceId) || this.instances.get(1) || this.instances.values().next().value;
        if (activeInstance) {
          const boostingCar = activeInstance.cars.find(c => c.state.isHyperBoosting);
          if (boostingCar && boostingCar.state) {
            ...
            const driftingCar = activeInstance.cars.find(c => c.state.isDrifting && Math.abs(c.state.driftAngle) > 0.35);
