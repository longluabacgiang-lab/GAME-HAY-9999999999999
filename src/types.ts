...
export interface SystemConfig {
  ...
  fps: FPSOption;
  carsPerRace?: 0 | 6 | 15 | 30 | number; // Số lượng xe đua: 0 = Random ngẫu nhiên (6-35 xe), hoặc 6, 15, 30 tay đua
...
export interface RacingCarAudioState {
  id: string;
  driverName?: string;
  speedKmh: number;
  rpm: number;
...
export interface RacingAudioFrameTelemetry {
  timeSec: number;
  cameraMode: CameraMode;
  cameraPos: { x: number; y: number; z: number };
  camForward: { x: number; y: number; z: number };
  cars: RacingCarAudioState[];
  isCollision?: boolean;
}
