...
import { StorylinesModal } from './components/StorylinesModal';
...
export default function App() {
  ...
  const [config, setConfig] = useState<SystemConfig>({
    ...
    carsPerRace: 0, // Mặc định: Random mỗi luồng 6 - 35 xe độc bản, không trùng lặp
  ...
  const [activeAudioInstanceId, setActiveAudioInstanceId] = useState<number>(1);
  const [isLibraryOpen, setIsLibraryOpen] = useState<boolean>(false);
  const [isLogsOpen, setIsLogsOpen] = useState<boolean>(false);
  const [isGuideOpen, setIsGuideOpen] = useState<boolean>(false);
  const [isAppleGameOpen, setIsAppleGameOpen] = useState<boolean>(false);
  const [isStorylinesOpen, setIsStorylinesOpen] = useState<boolean>(false);
  ...
  useEffect(() => {
    ...
    // Xử lý sự kiện khi một instance hoàn thành chu kỳ chặng đua (30s / 60s / 120s)
    engine.onChunkCompleted = (instance) => {
      addLog(
        'info',
        `Chặng đua ${config.durationSeconds || 120}s của Luồng #${instance.id} kết thúc! Đã chuyển vào Hàng đợi kết xuất MP4 60 FPS (Offline Simulation sắc nét 1080x1920 không xé hình).`,
        instance.id
      );
    ...
    // Đồng bộ HUD thời gian thực (giới hạn 150ms để tối ưu hóa CPU và duy trì 60 FPS mượt mà)
    let lastTick = 0;
    engine.onStateTick = () => {
  ...
  const handleApplyConfig = (updates: Partial<SystemConfig>) => {
    setConfig(prev => {
      ...
      const carsLabel = updated.carsPerRace === 0 ? 'Random mỗi luồng (6 - 35 xe độc bản)' : `${updated.carsPerRace || 15} xe`;
      addLog('success', `Đã áp dụng thông số: FPS = ${updated.fps}, Số xe đua = ${carsLabel}! Khi bấm Bắt Đầu Chạy, toàn bộ xe và đường đua sẽ chạy chính xác theo thông số này.`);
      return updated;
    });
  };
  ...
  const handleSelectCamera = (instanceId: number, mode: CameraMode) => {
    if (!engineRef.current) return;
    engineRef.current.setInstanceCamera(instanceId, mode, true);
    addLog('info', `Đã chuyển và khóa cố định Instance #${instanceId} ở góc quay: ${mode}`, instanceId);
  };

  const handleUnlockCamera = (instanceId: number) => {
    if (!engineRef.current) return;
    engineRef.current.unlockInstanceCamera(instanceId);
  ...
  const handleSelectAudioInstance = (instanceId: number) => {
    setActiveAudioInstanceId(instanceId);
    if (engineRef.current) {
      engineRef.current.setActiveAudioInstance(instanceId);
    }
    addLog('info', `Đã chuyển đổi âm thanh trực tiếp sang Luồng #${instanceId}`, instanceId);
  };
  ...
  return (
    <div className="flex flex-col h-screen w-screen overflow-hidden bg-slate-950 text-slate-100 font-sans select-none">
      ...
      <div className={`flex flex-col lg:flex-row flex-1 overflow-hidden relative ${appMode === 'VIDEO_FACTORY' ? 'flex z-10' : 'invisible absolute inset-0 -z-10 pointer-events-none'}`}>
        ...
        <main className="flex-1 relative flex flex-col overflow-hidden bg-black">
          ...
          <InstanceGrid
            ...
            activeAudioInstanceId={activeAudioInstanceId}
            onInspectInstance={id => {
              setInspectInstanceId(id);
              handleSelectAudioInstance(id);
            }}
            onSelectAudioInstance={handleSelectAudioInstance}
      ...
      <VideoFileManager
        ...
        onClearAllJobs={() => {
          videoRecorderService.clearAllJobs();
          setActiveJobs([]);
        }}
      />
      ...
      {/* Modal Kho 3,000 Cốt Truyện Kịch Bản Hài Hước */}
      <StorylinesModal
        isOpen={isStorylinesOpen}
        onClose={() => setIsStorylinesOpen(false)}
        onSelectStoryline={(storyline) => {
          addLog('success', `Đã kích hoạt kịch bản #${storyline.index}: ${storyline.titleVi}`);
          setIsStorylinesOpen(false);
        }}
      />
    </div>
  );
