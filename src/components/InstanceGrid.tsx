...
interface InstanceGridProps {
  ...
  activeCount: number;
  activeAudioInstanceId?: number;
  onInspectInstance: (instanceId: number) => void;
  onSelectAudioInstance?: (instanceId: number) => void;
...
export const InstanceGrid: React.FC<InstanceGridProps> = ({
  ...
  instances,
  activeCount,
  activeAudioInstanceId = 1,
  onInspectInstance,
  onSelectAudioInstance,
  onQuickSwitchCamera
}) => {
  ...
  const cameraModesList: CameraMode[] = [
    // Góc Quay Từ Xa & Truyền Hình F1 Live Show
  ...
  const getCameraLabelVN = (mode: CameraMode) => {
    switch (mode) {
      // Góc quay từ xa & Truyền hình
      case CameraMode.CHOPPER_HELI_CHASE: return 'Trực Thăng Truyền Hình';
      case CameraMode.SKY_DRONE_BROADCAST: return 'Drone Bay Siêu Tốc';
      case CameraMode.PANORAMIC: return 'Toàn Cảnh Trên Cao';
      case CameraMode.BEHIND: return 'Phía Sau Xe 100m';
      case CameraMode.MULTI_CAR_PACK_CHASE: return 'Bám Đuôi Đoàn Xe 100m';
      case CameraMode.MULTI_CAR_FRONT_FACING: return 'Đón Đầu Nhiều Xe';
      case CameraMode.MULTI_CAR_OVERTAKE_WIDE: return 'Toàn Cảnh So Kè Nhiều Xe';
      case CameraMode.SIDE_CHASE_MULTI: return 'Hông Xa So Kè Nhóm Xe';
      case CameraMode.TRACKSIDE_TELEPHOTO: return 'Telephoto Lia Xe (85mm)';
      case CameraMode.TRACKSIDE_APEX: return 'Mép Cua Apex Đón Xe';
      case CameraMode.PIT_WALL_BROADCAST: return 'Vách Pit Wall Lia Xe';
      case CameraMode.PASSING_STATIONARY: return 'Ven Rào Xé Gió';
      case CameraMode.VERTICAL_PORTRAIT_OPTIMIZED: return 'Dọc 9:16 TV';
      case CameraMode.SPECTATOR_TRACKSIDE: return 'Khán Giả Lia Theo Xe';
      case CameraMode.TUNNEL_CEILING_FAST: return 'Trần Hầm Siêu Tốc';
      case CameraMode.FENDER_WHEEL_LOOK: return 'Chắn Bùn Sát Lốp';
      case CameraMode.WING_REAR_LOOK: return 'Đuôi Gió Nhìn Trước';
      case CameraMode.KERB_CAM_GROUND: return 'Âm Vỉa Lên Gầm';

      // Cinematic car angles
      case CameraMode.HOOD: return 'Mui Xe / Capo';
      case CameraMode.LOW_GROUND: return 'Sát Mặt Đường (Giữa Vạch)';
      case CameraMode.SIDE_PROFILE: return 'Bên Hông Xe';
      case CameraMode.LEADER_TRACKING: return 'Bám Xe Dẫn Đầu (P1)';
      case CameraMode.OVERTAKE_ACTION: return 'Góc Vượt Mặt';
      case CameraMode.COLLISION_DRIFT: return 'Va Chạm & Drift';
      case CameraMode.CINEMATIC_ORBIT: return 'Xoay 360 Vòng Quanh Xe';
      case CameraMode.COCKPIT_FIRST_PERSON: return 'Góc Lái Thứ 1';
      case CameraMode.BUMPER_FIRST_PERSON: return 'Cản Trước Siêu Tốc';
      default: return 'Góc Quay Xe';
    }
  };
          ...
          return (
            ...
            >
              ...
              >
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-1">
                      ...
                      <button
                        onClick={(e) => {
                          e.stopPropagation();
                          onSelectAudioInstance?.(instance.id);
                        }}
                        className={`flex items-center gap-1 px-1.5 py-0.5 rounded text-[9px] font-mono font-bold border transition cursor-pointer ${
                          activeAudioInstanceId === instance.id
                            ? 'bg-emerald-500/30 text-emerald-300 border-emerald-500/60 shadow-sm'
                            : 'bg-slate-900/80 text-slate-400 border-slate-700 hover:text-cyan-300 hover:border-cyan-500/50'
                        }`}
                        title={
                          activeAudioInstanceId === instance.id
                            ? 'Âm thanh của luồng này đang phát trực tiếp ra loa'
                            : 'Bấm để nghe âm thanh riêng biệt của luồng này'
                        }
                      >
                        {activeAudioInstanceId === instance.id ? (
                          <>
                ...
                {/* Driver List Overlay - Professional Broadcast Starting Grid from Second 3 to Second 6 (Hiển thị đầy đủ tất cả tay đua, ví dụ 15 xe thì hiện đủ 15 tay đua) */}
                {instance.chunkTimeElapsed >= 3.0 && instance.chunkTimeElapsed <= 6.0 && (
                  <div className="absolute top-2 left-2 w-[245px] sm:w-[275px] max-h-[90%] bg-slate-950/95 backdrop-blur-md z-40 p-2 flex flex-col rounded-lg border border-cyan-500/40 shadow-2xl animate-fade-in font-sans pointer-events-none">
                    <div className="mb-1.5 border-b border-cyan-500/30 pb-1 shrink-0">
                      <div className="text-[7.5px] font-mono tracking-[0.1em] text-cyan-400 font-bold uppercase flex items-center gap-1">
                        <span className="w-1.5 h-1.5 rounded-full bg-cyan-400 animate-pulse"></span>
                        STARTING GRID • {instance.cars.length} TAY ĐUA
                      </div>
                      <h3 className="text-[9.5px] font-black text-white uppercase tracking-wide">
                        Danh Sách Tay Đua Xuất Phát
                      </h3>
                      <p className="text-[6.5px] text-slate-400 font-mono">
                        LUỒNG #{instance.id.toString().padStart(2, '0')} • CỰ LY HÀNG ĐÔI GRAND PRIX
                      </p>
                    </div>

                    <div className="grid grid-cols-2 gap-x-1.5 gap-y-0.5 overflow-y-auto max-h-[195px] pr-0.5">
                      {instance.cars.map((car, idx) => {
                        const bgHex = car.hexColor !== undefined
                          ? `#${car.hexColor.toString(16).padStart(6, '0')}`
                          : '#06b6d4';
                        const driverDisplayName = car.driverName || car.name || `Tay đua #${idx + 1}`;
                        const carTypeName = car.type === 'formula' ? 'F1' : car.type === 'hypercar' ? 'HYPER' : 'GT';

                        return (
                          <div
                            key={car.id || idx}
                            className="flex items-center justify-between bg-slate-900/90 border border-slate-800/80 rounded px-1 py-0.5 shrink-0"
                          >
                            <div className="flex items-center gap-1 min-w-0">
                              {/* Grid Position */}
                              <div className="w-3.5 h-3.5 rounded bg-cyan-950/80 border border-cyan-500/30 flex items-center justify-center font-mono font-black text-cyan-400 text-[7px] shrink-0">
                                {idx + 1}
                              </div>
                              ...
                              <div className="flex flex-col min-w-0 leading-tight">
                                <span className="font-bold text-white uppercase text-[7px] tracking-tight truncate max-w-[75px]">
                                  {driverDisplayName}
                                </span>
                                <span className="text-[5.5px] text-slate-400 font-mono truncate max-w-[75px]">
                                  {car.name || `#${idx + 1}`}
                                </span>
                              </div>
                            </div>
                            ...
                            {/* Category Badge */}
                            <div className="text-[5.5px] font-mono font-bold text-cyan-300 bg-cyan-950/60 px-0.5 rounded border border-cyan-500/20 shrink-0 ml-0.5">
                              {carTypeName}
                            </div>
                          </div>
                        );
