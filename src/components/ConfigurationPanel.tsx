...
import { Settings, Folder, Video, Sliders, Camera, Sparkles, FolderPlus, Download, Smartphone, Film, Check, Users, Gauge, Mic, MicOff, Globe, Volume2, Headphones, Wind } from 'lucide-react';
...
export const ConfigurationPanel: React.FC<ConfigurationPanelProps> = ({
  ...
  const [selectedCars, setSelectedCars] = useState<0 | 6 | 15 | 60 | number>((config.carsPerRace as any) ?? 0);
  ...
  const carScenarioOptions = [
    { count: 0, label: '🎲 Random', desc: 'Mỗi luồng ngẫu nhiên 6 đến 35 xe độc bản', badge: '6 - 35 Xe' },
    { count: 6, label: '6 Xe', desc: 'Đua nhóm tinh gọn, so kè nghẹt thở', badge: 'Sprint' },
    { count: 15, label: '15 Xe', desc: 'Chuẩn Grand Prix quốc tế, bám đuổi quyết liệt', badge: 'Grand Prix' },
    { count: 30, label: '30 Xe', desc: 'Đại chiến 30 tay đua cùng lúc', badge: 'Pro 30' }
  ];
...
`;
  ...
  return (
    <aside className="w-full lg:w-80 bg-slate-900/95 border-b lg:border-b-0 lg:border-r border-slate-800 p-4 space-y-4 text-sm text-slate-300 shrink-0 overflow-y-auto">
      ...
      <div className="space-y-1.5 p-2.5 rounded-xl bg-slate-950/70 border border-slate-800">
        <div className="flex items-center justify-between">
          ...
          <span className="text-[10px] font-mono font-bold text-amber-400 bg-amber-950/60 px-1.5 py-0.5 rounded border border-amber-500/40">
            {selectedCars === 0 ? '🎲 Random (8 - 16 Xe)' : `${selectedCars} Xe Cùng Lúc`}
          </span>
        </div>
        <div className="grid grid-cols-4 gap-1.5">
          {carScenarioOptions.map(opt => (
            ...
            >
              ...
              <span className="text-[9px] opacity-80 font-normal mt-0.5">{opt.badge}</span>
            </button>
          ))}
        </div>
        <p className="text-[10.5px] text-slate-400">
          {selectedCars === 0 
            ? 'Mỗi luồng khi chạy 6-8 luồng sẽ tự động phân bổ số lượng xe ngẫu nhiên (8 - 16 xe), không luồng nào giống luồng nào.'
      ...
      {/* 5. Bình Luận Viên Trực Tiếp Song Ngữ Việt - Anh */}
      <div className="space-y-2.5 p-3 rounded-xl bg-slate-950/70 border border-slate-800">
        ...
        <div className="space-y-1.5 pt-1">
          <label className="text-[11px] font-semibold text-slate-300 flex items-center gap-1">
            ...
            Ngôn Ngữ Bình Luận (Commentary Language):
          </label>
          <div className="grid grid-cols-2 gap-1.5">
            <button
              type="button"
              onClick={() => {
                commentaryEngine.setLanguage('vi');
                setCommentaryLang('vi');
              }}
            ...
            >
              <span>🇻🇳 Tiếng Việt (Trẻ Trâu)</span>
            </button>
            <button
              type="button"
              onClick={() => {
                commentaryEngine.setLanguage('en');
            ...
            >
              <span>🇬🇧 English (F1 Hype)</span>
            </button>
          </div>
        </div>

        {/* Nút nghe thử giọng đọc */}
        <button
          type="button"
          onClick={() => {
            if (!isCommentaryEnabled) {
              commentaryEngine.setEnabled(true);
              setIsCommentaryEnabled(true);
            }
            commentaryEngine.triggerEvent('OVERTAKE', undefined, true, { lang: commentaryLang });
          }}
          className="w-full flex items-center justify-center gap-1.5 py-1.5 px-3 rounded-lg bg-slate-900 hover:bg-slate-800 border border-slate-700 text-amber-300 hover:text-amber-200 text-xs font-bold transition active:scale-98 cursor-pointer"
          title="Nghe thử một câu bình luận trực tiếp theo ngôn ngữ đã chọn"
        >
          <Volume2 className="w-3.5 h-3.5 text-amber-400" />
          <span>🔊 {commentaryLang === 'en' ? 'Test English Commentary Voice' : 'Nghe thử câu bình luận Tiếng Việt'}</span>
        </button>

        <p className="text-[10.5px] text-slate-400">
          {commentaryLang === 'en'
            ? 'Bình luận viên F1 Viral tiếng Anh với hơn 150 tỷ tỷ biến thể, kết hợp 3,000 cốt truyện, hạ nhỏ tiếng động cơ (Audio Ducking) tự động.'
            : 'BLV AI tự động phát giọng đọc tiếng Việt và hiển thị phụ đề theo từng pha vượt mặt, ôm cua Drift, bứt tốc Nitro và về đích. Tiếng động cơ tự động hạ nhỏ (Audio Ducking).'}
        </p>
      </div>

      {/* 6. Đạo diễn âm thanh 3D Không gian & Môi trường Biome */}
      <div className="space-y-2.5 p-3 rounded-lg bg-gradient-to-b from-indigo-950/40 to-slate-950/80 border border-indigo-900/40">
        <div className="flex items-center justify-between">
          <label className="flex items-center gap-1.5 text-xs font-bold text-indigo-300">
            <Headphones className="w-4 h-4 text-indigo-400" />
            <span>Âm Thanh Không Gian 3D & Biome</span>
          </label>
          <span className="px-1.5 py-0.5 text-[9px] font-mono font-bold bg-indigo-500/20 text-indigo-300 border border-indigo-500/30 rounded uppercase">
            15 Xe Đa Âm • Foley Camera
          </span>
        </div>
        ...
        <div className="grid grid-cols-2 gap-1.5 text-[10.5px]">
          <div className="bg-slate-900/90 p-2 rounded border border-slate-800 text-slate-300">
            <div className="font-semibold text-white flex items-center gap-1 mb-0.5">
              <span>🚁 Foley Góc Quay</span>
            </div>
            <p className="text-[10px] text-slate-400">Trực thăng (cánh quạt đập), Drone (rít cao tần), Ven đường (vèo vèo), Cockpit (cách âm).</p>
          </div>
          <div className="bg-slate-900/90 p-2 rounded border border-slate-800 text-slate-300">
            <div className="font-semibold text-white flex items-center gap-1 mb-0.5">
              <span>🌧️ Môi Trường Biome</span>
            </div>
            <p className="text-[10px] text-slate-400">Tự động chuyển tiếng mưa bão, sấm sét, gió núi tuyết rít, bão cát sa mạc.</p>
          </div>
        </div>
        ...
        <button
          ...
          onClick={() => {
            audioEngine.init();
            audioEngine.triggerFlyby(520, -0.9, 0.95);
          }}
          className="w-full flex items-center justify-center gap-1.5 py-1.5 px-3 rounded-lg bg-indigo-950/60 hover:bg-indigo-900/60 border border-indigo-800/60 text-indigo-300 hover:text-indigo-200 text-xs font-bold transition active:scale-98 cursor-pointer"
          title="Nghe thử tiếng xe đua 500 km/h xé gió vụt qua camera ven đường"
        >
          <Wind className="w-3.5 h-3.5 text-indigo-400" />
          <span>⚡ Nghe thử tiếng xe xé gió vụt qua (Doppler Flyby)</span>
        </button>
      </div>

      {/* 7. Thư mục lưu trữ trên máy tính Windows */}
      <div className="space-y-2 p-3 rounded-lg bg-slate-950/80 border border-slate-800">
        ...
        <label className="flex items-center gap-2 pt-1 text-xs text-amber-300 font-semibold cursor-pointer select-none">
          <input
            type="checkbox"
            checked={config.autoDownloadToDevice ?? true}
            onChange={e => onChangeConfig({ autoDownloadToDevice: e.target.checked })}
            className="rounded text-amber-500 bg-slate-950 border-slate-700"
          />
          ⚡ Tự động tải video về máy ngay (Không cần ấn nút tải)
        </label>

        <label className="flex items-center gap-2 pt-0.5 text-xs text-slate-300 cursor-pointer select-none">
          <input
            type="checkbox"
          ...
          />
          Tự động xuất / tải file về máy mỗi khi đủ thời lượng
        </label>
      </div>
